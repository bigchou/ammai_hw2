...
class MetaTemplate(nn.Module):
    ...
    def parse_feature(self,x,is_feature):
        ...
        if is_feature:
            ...
            x           = x.contiguous().view( self.n_way * (self.n_support + self.n_query), *x.size()[2:])#100,3,84,84 
            z_all       = self.feature.forward(x)#(100,512)
            z_all       = z_all.view( self.n_way, self.n_support + self.n_query, -1)#(5,20,512)
        z_support   = z_all[:, :self.n_support]#(5,5,512)
        z_query     = z_all[:, self.n_support:]#(5,15,512)
