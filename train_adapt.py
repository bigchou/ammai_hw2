...
from methods.myTPNadapt import MyTPN_Adapt

from io_utils import model_dict, parse_args, get_resume_file, get_best_file, get_assigned_file
from datasets import miniImageNet_few_shot, ISIC_few_shot, EuroSAT_few_shot

def train(base_loader, val_loader, novel_loader, model, optimization, start_epoch, stop_epoch, params):    
    ...
    for epoch in range(start_epoch,stop_epoch):
        model.train()
        model.train_loop(epoch, base_loader, novel_loader, optimizer ) 
...
if __name__=='__main__':
    ...
    if params.method in ['baseline'] :
    ...
    elif params.method[:10] == 'mytpnadapt':
        print("INIT mytpnadapt")
        n_query = max(1, int(16* params.test_n_way/params.train_n_way)) #if test_n_way is smaller than train_n_way, reduce n_query to keep batch size small
        train_few_shot_params    = dict(n_way = params.train_n_way, n_support = params.n_shot) 
        test_few_shot_params     = dict(n_way = params.test_n_way, n_support = params.n_shot) 
        datamgr            = miniImageNet_few_shot.SetDataManager(image_size, n_query = n_query, mode="train",  **train_few_shot_params)
        ...
        if params.method == 'mytpnadaptisic':
            ...
            datamgr = ISIC_few_shot.SetDataManager(image_size,n_eposide=600, n_query=n_query,**few_shot_params)
        elif params.method == 'mytpnadapteurosat':
            print("init mytpnadapteurosat")
            datamgr = EuroSAT_few_shot.SetDataManager(image_size,n_eposide=600, n_query=n_query,**few_shot_params)
        else:
        ...
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, pretrained_dataset, params.model, params.method[:5])
        if params.train_aug:
            checkpoint_dir += '_aug'
        checkpoint_dir += '_5way_5shot'
        modelfile   = get_best_file(checkpoint_dir)
        print("load from %s"%(modelfile))
    ...
    model = train(base_loader, val_loader, novel_loader, model, optimization, start_epoch, stop_epoch, params)
